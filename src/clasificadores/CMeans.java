...
public class CMeans {
    ...
    public void clasifica (){
     ...
     for (int x=0; x < this.c;x++){
       ...
       centroides[x] = new Patron(this.instancias.get(pos).getVectorCa().clone(),"Centroide "+x);
     }
    ...
    private void inicializarNuevosCentroides(Patron[] centroidesNuevos) {
      ...
      for (int x=0; x < centroidesNuevos.length;x++){
        centroidesNuevos[x] = new Patron(new double[this.instancias.get(0).getVectorCa().length],this.getCentroides().get(this.getCentroides().size()-1)[x].getClase());
      }
    }
    ...
    public double calculaDistanciaEuclidiana(Patron p1,Patron p2){
     double acum = 0;
